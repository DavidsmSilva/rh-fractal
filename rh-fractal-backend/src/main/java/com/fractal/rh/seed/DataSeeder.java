...
public class DataSeeder implements CommandLineRunner {
  ...
  private void seedIfNeeded(String table) {
    ...
    if ("departamentos".equals(table)) {
      Departamento d1 = Departamento.builder().nombre("Tecnología").codigo("TEC").descripcion("Departamento encargado del desarrollo tecnológico, infraestructura y sistemas de información de la empresa").presupuesto(100000.0).jefe("Miguel Soto").numeroEmpleados(5).estado("ACTIVO").build();
      departamentoRepository.save(d1);
      Departamento d2 = Departamento.builder().nombre("Recursos Humanos").codigo("RH").descripcion("Gestión del talento humano, reclutamiento, selección y bienestar laboral").presupuesto(50000.0).jefe("Laura García").numeroEmpleados(3).estado("ACTIVO").build();
      departamentoRepository.save(d2);
      Departamento d3 = Departamento.builder().nombre("Mercadeo").codigo("MKT").descripcion("Estrategias de marketing, publicidad y comunicación corporativa").presupuesto(75000.0).jefe("Carlos Martínez").numeroEmpleados(4).estado("ACTIVO").build();
      departamentoRepository.save(d3);
      Departamento d4 = Departamento.builder().nombre("Finanzas").codigo("FIN").descripcion("Gestión financiera, contabilidad, presupuesto y control de costos").presupuesto(120000.0).jefe("Ana López").numeroEmpleados(4).estado("ACTIVO").build();
      departamentoRepository.save(d4);
      Departamento d5 = Departamento.builder().nombre("Operaciones").codigo("OPE").descripcion("Coordinación de procesos operativos y logística de la empresa").presupuesto(90000.0).jefe("Pedro Rodríguez").numeroEmpleados(6).estado("ACTIVO").build();
      departamentoRepository.save(d5);
      Departamento d6 = Departamento.builder().nombre("Administración").codigo("ADMIN").descripcion("Servicios generales, gestión documental y soporte administrativo").presupuesto(60000.0).jefe("Sofía Hernández").numeroEmpleados(3).estado("ACTIVO").build();
